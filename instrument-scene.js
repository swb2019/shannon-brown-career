...
export function createInstrument(canvas, dimensions, onFailure) {
  const renderer = new THREE.WebGLRenderer({canvas,alpha:true,antialias:true,powerPreference:'low-power'});
  renderer.setPixelRatio(Math.min(dimensions.pixelRatio,1.5));
  ...
  function fail(){if(stopped)return;stopped=true;cancelAnimationFrame(frame);renderer.dispose();environment.dispose();onFailure();}
  canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();fail();},{once:true});
  function size(){if(stopped)return;const b=dimensions;if(!b.width||!b.height)return;renderer.setSize(b.width,b.height,false);camera.aspect=b.width/b.height;camera.updateProjectionMatrix();draw();}
  function draw(){if(stopped)return;try{renderer.render(scene,camera);}catch{fail();}}
  function tick(now){
    frame=0;if(stopped||paused||!visible)return;
    if(!start)start=now;
  ...
  function run(){if(!frame&&!stopped&&!paused&&visible){previous=0;frame=requestAnimationFrame(tick);}}
  function stop(){cancelAnimationFrame(frame);frame=0;previous=0;}
  size();run();
  return {
    resize(value){dimensions=value;size();},
    setVisible(value){visible=value;value?run():stop();},
    dispose:fail,
